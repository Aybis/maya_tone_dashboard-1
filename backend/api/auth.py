...
def login():
    ...
    try:
        ...
        if response.status_code == 401:
            ...
            return (
                jsonify(
                    {
                        "success": False,
                        "error": "Unable to connect to Jira. Please try again later.",
                    }
                ),
                500,
            )
        ...
        session["jira_display_name"] = user_info.get("displayName", username)
        ...
        return (
            jsonify(
                {
                    "success": False,
                    "error": "Unable to connect to Jira. Please check your network connection.",
                }
            ),
            500,
        )
...
def check_auth():
    ...
    if session.get("logged_in"):
        return jsonify(
            {"authenticated": True, "username": session.get("jira_display_name", session.get("jira_username"))}
        )
