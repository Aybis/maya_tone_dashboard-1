...
def aggregate():
    ...
    if filters.get('status'):
        ...
        if q: clauses.append(f'status in ({q})')
    if filters.get('assignee'):
        q = quote_list(filters['assignee'])
        if q: clauses.append(f'assignee in ({q})')
    if filters.get('project'):
        q = quote_list(filters['project'])
        if q: clauses.append(f'project in ({q})')
    jql_extra = ' AND '.join(clauses) if clauses else None

    agg, err = aggregate_issues(_mgr(), group_by=group_by, from_date=from_date, to_date=to_date, jql_extra=jql_extra)
    if err:
    ...
    palette = ['#3b82f6','#10b981','#f59e0b','#ef4444','#8b5cf6','#06b6d4','#84cc16','#6366f1','#d946ef','#f87171','#0ea5e9','#64748b']
