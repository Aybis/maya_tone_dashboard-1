...
export default function Canvas() {
    ...
    socketRef.current.on('new_message', (data) => {
      if (data.chat_id === activeChatId) {
        setMessages((prev) => {
          ...
          // Parse message for export data
          let messageObj = { sender: data.sender, content: data.content };
          if (data.sender === 'assistant') {
            try {
              // Check if the message contains export data (download_link and filename)
              const exportMatch = data.content.match(/\[EXPORT_DATA\](.*?)\[\/EXPORT_DATA\]/s);
              if (exportMatch) {
                const exportData = JSON.parse(exportMatch[1]);
                if (exportData.download_link && exportData.filename) {
                  messageObj.downloadData = exportData;
                  // Remove the export data from the visible content
                  messageObj.content = data.content.replace(/\[EXPORT_DATA\].*?\[\/EXPORT_DATA\]/s, '');
                }
              }
            } catch (e) {
    ...
    socketRef.current.on('assistant_end', ({ chat_id, content }) => {
      if (chat_id === activeChatId) {
        setMessages((prev) => {
          ...
          for (let i = copy.length - 1; i >= 0; i--) {
            if (copy[i].sender === 'assistant') {
              let messageObj = { ...copy[i], content };

              // Parse final content for export data
              try {
                const exportMatch = content.match(/\[EXPORT_DATA\](.*?)\[\/EXPORT_DATA\]/s);
                if (exportMatch) {
                  const exportData = JSON.parse(exportMatch[1]);
                  if (exportData.download_link && exportData.filename) {
                    messageObj.downloadData = exportData;
                    // Remove the export data from the visible content
                    messageObj.content = content.replace(/\[EXPORT_DATA\].*?\[\/EXPORT_DATA\]/s, '');
                  }
                }
              } catch (e) {
                // If parsing fails, just use the original content
  ...
  const latestAssistant = useMemo(() => {
    return [...messages].reverse().find((m) => {
      ...
      const hasDownloadData = m.downloadData && m.downloadData.download_link;
      return hasTable || hasChartFence || hasDownloadData;
    });
  }, [messages]);
        ...
        <div className="space-y-6">
          {latestAssistant ? (
            <div className="bg-[#0f0f23]/70 border border-blue-500/10 rounded-lg p-4 space-y-4">
              ...
              />
              {/* Download button for export_worklog_data */}
              {latestAssistant?.downloadData && (
                <div className="mt-4 p-3 bg-slate-800/50 rounded-lg border border-blue-500/20">
                  <div className="flex items-center gap-3">
                    <div className="text-sm text-slate-300">
                      <span className="font-medium">Export Ready:</span> {latestAssistant.downloadData.filename}
                    </div>
                    <button
                      onClick={() => {
                        const link = document.createElement('a');
                        link.href = latestAssistant.downloadData.download_link;
                        link.download = latestAssistant.downloadData.filename;
                        document.body.appendChild(link);
                        link.click();
                        document.body.removeChild(link);
                      }}
                      className="px-4 py-2 bg-gradient-to-r from-green-500 to-green-700 text-white text-sm rounded-lg hover:from-green-400 hover:to-green-600 transition-all duration-200 flex items-center gap-2"
                    >
